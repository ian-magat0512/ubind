// <copyright file="DateTimeInPeriodFilterProviderConfigModelTests.cs" company="uBind">
...
namespace UBind.Application.Tests.Automations.Filters
{
    ...
    public class DateTimeInPeriodFilterProviderConfigModelTests
    {
        [Theory]
...
{{
    ""dateTimeIsInPeriodCondition"": {{
        ...
        ""isInPeriod"": {{
            ...
            var scope = new ExpressionScope("foo", Expression.Parameter(typeof(int)));
            ...
            var expression = (LambdaExpression)await provider.Resolve(new ProviderContext(automationData), scope);

            // Assert
            expression.Compile().DynamicInvoke(1).Should().Be(expectedResult);
        }
    }
}
