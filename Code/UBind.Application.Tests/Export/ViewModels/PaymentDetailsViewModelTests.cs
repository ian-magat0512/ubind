// <copyright file="PaymentDetailsViewModelTests.cs" company="uBind">
...
namespace UBind.Application.Tests.Export.ViewModels
{
    ...
    {
        [Fact]
        public void Request_ReturnsCorrectNestedData_WhenJsonPathIsCorrect()
        {
            // Arrange
            var request = "{}";
            var response = this.GetSampleResponseJson();
            var paymenDetails = new PaymentDetails(PaymentGatewayName.None, 100m, "Foo", request, response);
            var paymentAttemptResult = PaymentAttemptResult.CreateSuccessResponse(
                paymenDetails, null, SystemClock.Instance.Now());
            var sut = new PaymentDetailsViewModel(paymentAttemptResult);

            // Act
            var last4digits = sut.Response["$.payment_method_details.card.last4"];

            // Assert
            Assert.Equal("4242", last4digits);
        }

        private string GetSampleResponseJson()
        {
            return @"{
