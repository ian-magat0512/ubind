...
namespace UBind.Application.Tests.ThirdPartyDataSets.Commands.Gnaf
{
    ...
    public class CreateTablesAndSchemaCommandHandlerTest : IClassFixture<ThirdPartyDataSetsTestFixture>
    {
        ...
        public CreateTablesAndSchemaCommandHandlerTest(ThirdPartyDataSetsTestFixture thirdPartyDataSetsTestFixture)
        {
            ...
            services.AddSingleton(new Mock<IGnafRepository>().Object);
        ...
        public async Task Handle_ReturnsUnitValue_WhenTablesAndSchemasAreCreated()
        {
            ...
            mockFileSystem.Setup(setup => setup.Directory.GetFiles(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<SearchOption>())).Returns(
                new[]
                {
                    "c://createTableScript.sql",
                });

            mockFileSystem.Setup(setup => setup.File.ReadAllText(It.IsAny<string>())).Returns("CREATE TABLE [dbo].[Foo]([fooField] [varchar](25)) ");
