...
namespace UBind.Application.Tests.ThirdPartyDataSets.GlassGuide.UnitTest;
...
public class LocalIntegrationTest : IClassFixture<ThirdPartyDataSetsTestFixture>
{
    ...
    public LocalIntegrationTest(ThirdPartyDataSetsTestFixture thirdPartyDataSetsTestFixture)
    {
        ...
        services.AddSingleton<IGlassGuideRepository, GlassGuideRepository>();
    ...
    public async Task Handle_ReturnsDownloadLocationFolder_WhenFileIsDownloadedViaFTP()
    {
        ...
        Assert.NotNull(sut);
        Assert.NotNull(updaterJobManifest);

        //// Act
        var result = await sut.Handle(new DownloadFilesCommand(updaterJobManifest, this.thirdPartyDataSetsTestFixture.JobId, UpdaterJobType.GlassGuide), CancellationToken.None);
    ...
    public async Task Handle_ReturnsUnitValue_WhenTablesAndSchemasAreCreated()
    {
        ...
        Assert.NotNull(thirdPartyDataSetsDbContext);
        ...
        var glassGuideRepository = service.GetService<IGlassGuideRepository>();
        Assert.NotNull(glassGuideRepository);
        await glassGuideRepository.DropAllTablesByIndex(Schema.GlassGuideStaging.Humanize());

        /////////// Act
        var result = await sut.Handle(new CreateTablesAndSchemaCommand(Schema.GlassGuideStaging), CancellationToken.None);
    ...
    public async Task Handle_ReturnsUnitValue_WhenImportingDelimiterSeparatedValues()
    {
        ...
        /////////// Act
        var result = await sut.Handle(new ImportDelimiterSeparatedValuesCommand(this.thirdPartyDataSetsTestFixture.JobId), CancellationToken.None);
    ...
    public async Task Handle_ReturnsListOfVehicleMakes_GetVehicleMakesQuery()
    {
        //////////// Arrange
        var service = this.serviceCollection.BuildServiceProvider();
        var sut = service.GetService<IQueryHandler<Application.Queries.ThirdPartyDataSets.GlassGuide.GetVehicleMakesQuery, IEnumerable<VehicleMake>>>();

        Assert.NotNull(sut);

        /////////// Act
        var result = await sut.Handle(new Application.Queries.ThirdPartyDataSets.GlassGuide.GetVehicleMakesQuery(2020), CancellationToken.None);
