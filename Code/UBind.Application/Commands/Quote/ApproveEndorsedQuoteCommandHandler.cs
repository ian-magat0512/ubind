// <copyright file="ApproveEndorsedQuoteCommandHandler.cs" company="uBind">
...
public class ApproveEndorsedQuoteCommandHandler : ICommandHandler<ApproveEndorsedQuoteCommand, NewQuoteReadModel>
{
    ...
    public ApproveEndorsedQuoteCommandHandler(
    ...
    public async Task<NewQuoteReadModel> Handle(ApproveEndorsedQuoteCommand command, CancellationToken cancellationToken)
    {
        ...
        using (await this.aggregateLockingService.CreateLockOrThrow(command.TenantId, quoteAggregateId, AggregateType.Quote))
        {
            ...
            await this.quoteEndorsementService.ApproveEndorsedQuote(releaseContext, quote, command.FormData);
