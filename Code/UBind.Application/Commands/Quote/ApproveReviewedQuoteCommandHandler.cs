// <copyright file="ApproveReviewedQuoteCommandHandler.cs" company="uBind">
...
public class ApproveReviewedQuoteCommandHandler : ICommandHandler<ApproveReviewedQuoteCommand, NewQuoteReadModel>
{
    ...
    public ApproveReviewedQuoteCommandHandler(
    ...
    public async Task<NewQuoteReadModel> Handle(ApproveReviewedQuoteCommand command, CancellationToken cancellationToken)
    {
        ...
        using (await this.aggregateLockingService.CreateLockOrThrow(command.TenantId, quoteAggregateId, AggregateType.Quote))
        {
            ...
            await this.quoteEndorsementService.ApproveReviewedQuote(releaseContext, quote, command.FormData);
