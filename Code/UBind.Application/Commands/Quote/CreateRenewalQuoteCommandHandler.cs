// <copyright file="CreateRenewalQuoteCommandHandler.cs" company="uBind">
...
public class CreateRenewalQuoteCommandHandler : ICommandHandler<CreateRenewalQuoteCommand, NewQuoteReadModel>
{
    ...
    private readonly IPolicyTransactionTimeOfDayScheme policyLimitTimesPolicy;
    ...
    public CreateRenewalQuoteCommandHandler(
        ...
        Domain.Services.IPolicyService policyService,
        ...
        IAggregateLockingService aggregateLockingService)
    {
        ...
        this.policyLimitTimesPolicy = policyLimitTimesPolicy;
    ...
    public async Task<NewQuoteReadModel> Handle(CreateRenewalQuoteCommand command, CancellationToken cancellationToken)
    {
        ...
        using (await this.aggregateLockingService.CreateLockOrThrow(command.TenantId, quoteAggregateId, AggregateType.Quote))
        {
            ...
            bool isMutual = TenantHelper.IsMutual(tenant.Details.Alias);
            entityName = TenantHelper.CheckAndChangeTextToMutual("policy", isMutual);
            if (quoteAggregate.TenantId != command.TenantId)
            {
                throw new ErrorException(Errors.General.NotAuthorized("create a renewal quote", entityName, command.PolicyId));
            }
            ...
            if (!productFeature.AreRenewalQuotesEnabled)
            {
                throw new ErrorException(Errors.ProductFeatureSetting.QuoteTypeDisabled(quoteAggregate.ProductId, product.Details.Name, "Renewal"));
            }
            ...
            var renewalQuote = policy.CreateRenewalQuote(
                pastClaims,
                this.clock.Now(),
                quoteNumber,
                this.httpContextPropertiesResolver.PerformingUserId,
                quoteWorkflow,
                this.quoteExpirySettingsProvider.Retrieve(product),
