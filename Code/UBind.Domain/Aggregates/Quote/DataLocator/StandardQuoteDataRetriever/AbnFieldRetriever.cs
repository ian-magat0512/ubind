// <copyright file="AbnFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// </summary>
    public class AbnFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.Abn, config.QuoteDataLocations?.Abn);
