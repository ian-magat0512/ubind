// <copyright file="CustomerEmailFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// <summary>
    /// This class is needed to retrieve the customer email data from formdata or calculation.
    /// </summary>
    public class CustomerEmailFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.CustomerEmail, config.QuoteDataLocations?.ContactEmail);
