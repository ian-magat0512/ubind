// <copyright file="CustomerPhoneFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// <summary>
    /// This class is needed to retrieve the customer phone data from formdata or calculation.
    /// </summary>
    public class CustomerPhoneFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.CustomerPhone, config.QuoteDataLocations?.ContactPhone);
