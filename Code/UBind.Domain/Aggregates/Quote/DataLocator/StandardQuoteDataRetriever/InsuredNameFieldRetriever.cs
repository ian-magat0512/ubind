// <copyright file="InsuredNameFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// <summary>
    /// This class is needed to retrieve the insured name data from formdata or calculation.
    /// </summary>
    public class InsuredNameFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.InsuredName, config.QuoteDataLocations?.InsuredName);
