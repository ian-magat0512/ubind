// <copyright file="IsRefundApprovedFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// <summary>
    /// This class is needed to retrieve the cancellation refund selection data from formdata or calculation.
    /// </summary>
    public class IsRefundApprovedFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.IsRefundApproved, config.QuoteDataLocations?.IsRefundApproved);
