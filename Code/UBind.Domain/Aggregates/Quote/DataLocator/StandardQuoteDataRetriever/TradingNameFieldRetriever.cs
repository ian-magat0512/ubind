// <copyright file="TradingNameFieldRetriever.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote.DataLocator.StandardQuoteDataRetriever
{
    ...
    /// <summary>
    /// This class is needed to retrieve the trading name data from formdata or calculation.
    /// </summary>
    public class TradingNameFieldRetriever : BaseFieldRetriever
    {
        /// <inheritdoc/>
        public override object Retrieve(IDataLocatorConfig config, CachingJObjectWrapper formData, CachingJObjectWrapper calculationData)
        {
            ...
            var dataLocations = this.GetDataLocations(config.DataLocators?.TradingName, config.QuoteDataLocations?.TradingName);
