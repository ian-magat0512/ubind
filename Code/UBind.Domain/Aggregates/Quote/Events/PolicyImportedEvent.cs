// <copyright file="PolicyImportedEvent.cs" company="uBind">
...
namespace UBind.Domain.Aggregates.Quote
{
    ...
    public partial class QuoteAggregate
    {
        ...
        /// </summary>
        public class PolicyImportedEvent : Event<QuoteAggregate, Guid>, IPolicyUpsertEvent, IPolicyCreatedEvent
        {
            ...
            /// <summary>
            /// Initializes a new instance of the <see cref="PolicyImportedEvent"/> class.
            /// </summary>
            ...
            /// <param name="data">The policy import data.</param>
                ...
                : base(tenantId, aggregateId, performingUserId, createdTimestamp)
            {
                ...
                this.InceptionDateTime = data.InceptionDate.ToLocalDateFromIso8601OrddMMyyyyOrddMMyy(nameof(data.InceptionDate))
                    .At(timeOfDayScheme.GetInceptionTime());
                this.InceptionTimestamp = this.InceptionDateTime.InZoneLeniently(timeZone).ToInstant();
                this.ExpiryDateTime = data.ExpiryDate.ToLocalDateFromIso8601OrddMMyyyyOrddMMyy(nameof(data.ExpiryDate))
                    .At(timeOfDayScheme.GetEndTime());
                ...
                var calculationResult = ReadWriteModel.CalculationResult
                    .CreateForNewPolicy(new CachingJObjectWrapper(data.CalculationResult), quoteDataRetriever);

                var dataSnapshot = new QuoteDataSnapshot(
                    new QuoteDataUpdate<FormData>(
                        aggregateId, new FormData(data.FormData), createdTimestamp),
                    new QuoteDataUpdate<ReadWriteModel.CalculationResult>(aggregateId, calculationResult, createdTimestamp),
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            private void SetOwnership(UserReadModel? owner)
            {
                if (owner != null)
                {
                    this.OwnerUserId = owner.Id;
                    this.OwnerPersonId = owner.PersonId;
