...
import formConfig from './buttons.test-form-config.json';
...
describe('ButtonsField', () => {
    ...
    it('when a Button Field\'s option icon changes, the new icon is rendered', async () => {
        // Arrange
        ...
        response2.form.optionSets[0].options[0].icon = 'fa fa-laptop';
            ...
            eventService.webFormLoadedSubject
                ...
                .subscribe(async () => {
                    let debugElement: DebugElement = fixture.debugElement.query(
                        By.css('formly-group formly-field:first-child label:first-child .option-icon span'));
                    let spanEl: HTMLElement = debugElement.nativeElement;
                    expect(spanEl.classList.contains('fa-car'))
                        .toBeTruthy('The first button should initially be fa-car');
                    ...
                    debugElement = fixture.debugElement.query(
                        By.css('formly-group formly-field:first-child label:first-child .option-icon span'));
                    spanEl = debugElement.nativeElement;
                    expect(spanEl.classList.contains('fa-laptop'))
                        .toBeTruthy('The first button should now be fa-laptop');
                    expect(spanEl.classList.contains('fa-car'))
                        .toBeFalsy('The first button no longer be fa-car');
                    resolve();
                });
        });
    });

    it('when a Button Field\'s option hideCondition is set, that option is no longer displayed', async () => {
        // Arrange
        fixture = TestBed.createComponent(TestHostComponent);
        sut = fixture.componentInstance;
        eventService = TestBed.inject<EventService>(EventService);
        ...
        response1['status'] = 'success';
        let response2: any = _.cloneDeep(response1);
        response2.form.optionSets[0].options[0].hideConditionExpression = 'true';
        let configProcessorService: ConfigProcessorService
            = TestBed.inject<ConfigProcessorService>(ConfigProcessorService);
        configProcessorService.onConfigurationResponse(response1);
        sut.ready = true;
            ...
            eventService.webFormLoadedSubject
                ...
                .subscribe(async () => {
                    ...
                    debugElement = fixture.debugElement.query(
                        By.css('formly-group formly-field:first-child label:first-child .option-icon span'));
                    spanEl = debugElement.nativeElement;
                    expect(spanEl.classList.contains('fa-key'))
                        .toBeTruthy('The first button should now be fa-key');
                    expect(spanEl.classList.contains('fa-car'))
                        .toBeFalsy('The first button should no longer be fa-car');
                    resolve();
                });
        });
    });
