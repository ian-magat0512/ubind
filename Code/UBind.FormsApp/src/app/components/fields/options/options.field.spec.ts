...
import formConfig from './options-field.test-form-config.json';
...
describe('OptionsField', () => {
    ...
    it('when a field\'s fieldType is changed from Buttons to Select, it changes', async () => {
        // Arrange
        ...
        response2.form.questionSets[0].fields[0].$type = 'drop-down-select';
            ...
            eventService.webFormLoadedSubject
                ...
                .subscribe(async () => {
                    let debugElement: DebugElement
                        = fixture.debugElement.query(By.css('.key-cars buttons-field'));
                    expect(debugElement != null)
                        .toBeTruthy('The options field should have been rendered as a buttons field from the start');

                    // Act
                    configProcessorService.onConfigurationResponse(response2);
                    fixture.detectChanges();

                    debugElement = fixture.debugElement.query(By.css('.key-cars select-field'));
                    expect(debugElement != null)
                        .toBeTruthy('The options field should have been changed to drop down select field');
                    resolve();
                });
        });
    });

    it('when a field\'s option set is changed, it displays different options', async () => {
        // Arrange
        fixture = TestBed.createComponent(TestHostComponent);
        sut = fixture.componentInstance;
        eventService = TestBed.inject<EventService>(EventService);
        let response1: any = _.cloneDeep(formConfig);
        response1['status'] = 'success';
        response1.form.workflowConfiguration.step1.articles[0].elements.pop();
        let response2: any = _.cloneDeep(response1);
        response2.form.questionSets[0].fields[0].optionSetName = 'Fast Cars';
        response2.form.questionSets[0].fields[0].optionSetKey = 'fastCars';
        let configProcessorService: ConfigProcessorService
            = TestBed.inject<ConfigProcessorService>(ConfigProcessorService);
        configProcessorService.onConfigurationResponse(response1);
        sut.ready = true;
        ...
        return new Promise((resolve: any, reject: any): void => {
            eventService.webFormLoadedSubject
                ...
                .subscribe(async () => {
                    let debugElement: DebugElement
                        = fixture.debugElement.query(By.css('.key-cars buttons-field .toy'));
                    expect(debugElement != null)
                        .toBeTruthy('The options field should have rendered a Toyota option');

                    // Act
                    configProcessorService.onConfigurationResponse(response2);
                    fixture.detectChanges();

                    debugElement = fixture.debugElement.query(By.css('.key-cars buttons-field .astonMartin'));
                    expect(debugElement != null)
                        .toBeTruthy('The options field should have rendered an Aston Martin option');
                    debugElement = fixture.debugElement.query(By.css('.key-cars buttons-field .hol'));
                    expect(debugElement == null)
                        .toBeTruthy('The options field should not have rendered a Holden option');
                    resolve();
                });
        });
    });
