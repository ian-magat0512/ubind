...
import formConfig from './select.test-form-config.json';
...
describe('SelectField', () => {
    ...
    it('when a Select List Field\'s option disabledCondition is set, that option becomes disabled', async () => {
        // Arrange
        ...
        response2.form.optionSets[0].options[2].disabledConditionExpression = 'true';
            ...
            eventService.webFormLoadedSubject
                ...
                .subscribe(async () => {
                    let debugElement: DebugElement = fixture.debugElement.query(
                        By.css('formly-group formly-field:first-child select option:last-child'));
                    let spanEl: HTMLElement = debugElement.nativeElement;
                    expect(spanEl.attributes.getNamedItem('disabled') == null)
                        .toBeTruthy('The option should not initially be disabled');
                    ...
                    expect(spanEl.attributes.getNamedItem('disabled') != null)
                        .toBeTruthy('The option should become disabled');
                    resolve();
                });
        });
    });
