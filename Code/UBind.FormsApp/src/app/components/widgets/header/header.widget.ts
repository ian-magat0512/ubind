 ...
 */
@Component({
    selector: 'ubind-header-widget',
    templateUrl: './header.widget.html',
    styleUrls: ['./header.widget.scss'],
})
export class HeaderWidget extends ContentListWidget implements OnInit {
    ...
    protected getContentDefinitions(): Array<ContentDefinition> {
        if (this.configService.workflowSteps[this.currentWorkflowStepName]) {
            return this.configService.workflowSteps[this.currentWorkflowStepName].header;
        } else {
            // TODO: When we define header elements at the top level (above the workflow step)
