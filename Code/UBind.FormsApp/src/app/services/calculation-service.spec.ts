...
import calculationResponse from './calculation-service.test-calculation-response.json';
import formConfig from './test/form-config1.json';
...
describe('CalculationService', () => {
    ...
    it('makes the quoteState available to the getQuoteState expression method after processing a calculation result',
        () => {
        // Arrange
            const expressionMethodService: ExpressionMethodService =
            new ExpressionMethodService(applicationService, null, null, null, null, null,
                null, null, workflowStatusService, null, null, null, null, null, null, null, null, null, null);
            let calculationService: CalculationService = TestBed.inject<CalculationService>(CalculationService);
            let calcResponse: any = _.cloneDeep(calculationResponse);
            let configurationResponse: any = _.cloneDeep(formConfig);
            configurationResponse['status'] = 'success';
            let configProcessorService: ConfigProcessorService
                = TestBed.inject<ConfigProcessorService>(ConfigProcessorService);
            configProcessorService.onConfigurationResponse(configurationResponse);
            // eslint-disable-next-line no-unused-vars
            let quoteResultProcessor: QuoteResultProcessor = TestBed.inject<QuoteResultProcessor>(QuoteResultProcessor);

            // Act
            calculationService.processCalculationResponse(calcResponse);

            // Assert
            expect(expressionMethodService.getQuoteState()).toBe(QuoteState.Approved);
        });
