// <copyright file="TextStartsWithFilterProviderConfigModelIntegrationTests.cs" company="uBind">
...
namespace UBind.Persistence.Tests.IntegrationTests.Application.Automations.Filters
{
    ...
    public class TextStartsWithFilterProviderConfigModelIntegrationTests : TextFilterProviderConfigModelIntegrationTests
    {
        [Theory]
        ...
        [InlineData("abcdef", " ", false)]
        [InlineData(" abcdef", "  ", false)]
        [InlineData("abc def ", "d", false)]
        public async Task Subject_CorrectlyFiltersAsync(string text, string substring, bool expectedResult)
        {
            await this.VerifyTextFilterBehaviourAsync("textStartsWithCondition", text, "startsWith", substring, expectedResult);
        }
    }
}
